...
public class Functions
{
	...
	public static Step[] findPath(String startingID, String endingID, int time, String day, Connection conn)
	{
		...
		try
		{
			...
			connectedQuery = conn.prepareStatement("select destination from connections where source = ? and source_time >= ? and " + day + " = 1 order by source_time asc, destination_time asc;");
			// gets soonest connection between source and destination after a certain time
			timesQuery = conn.prepareStatement("select source_time, destination_time from connections where source = ? and destination = ? and source_time >= ? and " + day + " = 1 order by source_time asc, destination_time asc limit 1;");
			visited.clear();
